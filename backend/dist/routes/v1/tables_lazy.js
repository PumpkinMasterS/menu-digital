...
const tableCreateSchema = zod_1.z.object({
    name: zod_1.z.string().min(1),
    code: zod_1.z.string().min(1),
    type: zod_1.z.enum(['table', 'takeaway']).optional().default('table'),
    isActive: zod_1.z.boolean().optional().default(true),
}).passthrough();
const tableUpdateSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).optional(),
    code: zod_1.z.string().min(1).optional(),
    type: zod_1.z.enum(['table', 'takeaway']).optional(),
    isActive: zod_1.z.boolean().optional(),
}).passthrough();
const tablesRoutes = async (app) => {
    // Admin: listar mesas
    app.get('/v1/admin/tables', async (req, reply) => {
        try {
            const tablesCol = await (0, db_1.getCollection)('tables');
    ...
    app.patch('/v1/admin/tables/:id', async (req, reply) => {
        try {
            ...
            const tablesCol = await (0, db_1.getCollection)('tables');
            const now = new Date().toISOString();
            const updated = await tablesCol.findOneAndUpdate({ id }, { $set: { ...parse.data, updatedAt: now } }, { returnDocument: 'after' });
            if (!updated.value)
    ...
    app.get('/v1/admin/tables/:id/qrcode', async (req, reply) => {
        try {
            ...
            const svg = await new Promise((resolve, reject) => {
                qrcode_1.default.toString(url, { type: 'svg' }, (err, data) => {
                    if (err)
                        reject(err);
                    else
                        resolve(data);
