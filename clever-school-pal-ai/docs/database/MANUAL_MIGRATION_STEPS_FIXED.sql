-- ===================================================================
-- MANUAL MIGRATION FOR CONTENT ORGANIZATION FEATURES (FIXED)
-- ===================================================================
...
-- STEP 8: Insert sample topics for existing subjects (FIXED - handles grades outside 5-9 range)
...
BEGIN
  IF EXISTS (SELECT 1 FROM subjects LIMIT 1) THEN
    ...
    SELECT 
      ...
      -- Fixed: Cap year_level to valid range (5-9)
      CASE 
        WHEN CAST(NULLIF(regexp_replace(s.grade, '[^0-9]', '', 'g'), '') AS INTEGER) < 5 THEN 5
        WHEN CAST(NULLIF(regexp_replace(s.grade, '[^0-9]', '', 'g'), '') AS INTEGER) > 9 THEN 9
        ELSE COALESCE(
    ...
    AND CAST(NULLIF(regexp_replace(s.grade, '[^0-9]', '', 'g'), '') AS INTEGER) IS NOT NULL
    ON CONFLICT DO NOTHING;
    
    -- Also handle subjects with non-numeric grades
    INSERT INTO topics (name, description, year_level, subject_id, order_index) 
    SELECT 
      CASE 
        WHEN s.name ILIKE '%matemática%' OR s.name ILIKE '%math%' THEN 'Números e Operações'
      ...
      END as name,
      'Tópico fundamental da disciplina' as description,
      6 as year_level, -- Default to 6th grade for non-numeric grades
      s.id as subject_id,
      1 as order_index
    FROM subjects s
    WHERE NOT EXISTS (
    ...
    AND (CAST(NULLIF(regexp_replace(s.grade, '[^0-9]', '', 'g'), '') AS INTEGER) IS NULL
         OR s.grade IS NULL)
    ON CONFLICT DO NOTHING;
...
-- STEP 9: Update existing contents with appropriate year_level based on subject grade (FIXED)
-- This sets year_level based on the grade field in the subjects table, capped to 5-9 range
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM contents LIMIT 1) THEN
    ...
    AND (year_level IS NULL OR year_level = 5);
    
    RAISE NOTICE 'Updated existing contents with year_level (capped to 5-9 range)';
  ELSE
...
-- Check year_level distribution in topics
SELECT year_level, COUNT(*) as count 
FROM topics 
GROUP BY year_level 
ORDER BY year_level;

-- Check subjects with grades outside 5-9 range
SELECT grade, COUNT(*) as count 
FROM subjects 
WHERE CAST(NULLIF(regexp_replace(grade, '[^0-9]', '', 'g'), '') AS INTEGER) IS NOT NULL
GROUP BY grade 
ORDER BY CAST(NULLIF(regexp_replace(grade, '[^0-9]', '', 'g'), '') AS INTEGER);
