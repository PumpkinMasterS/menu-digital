...
// Simple in-memory TTL cache for Node runtime
class TTLCache {
...
export function createContextRegistry(supabase: SupabaseClient, options: RegistryOptions = {}) {
  ...
  async function getGlobalContext(): Promise<GlobalContext> {
    ...
    const cached = cache.get<GlobalContext>(cacheKey);
    ...
    cache.set(cacheKey, globalCtx, ttl);
    return globalCtx;
  }
  ...
  async function getSchoolContext(schoolId: string): Promise<SchoolContext | undefined> {
    ...
    const cached = cache.get<SchoolContext>(cacheKey);
    if (cached) return cached;
    ...
    // Optional school guidelines/personality from school_context table
    ...
    cache.set(cacheKey, schoolCtx, ttl);
    return schoolCtx;
  }
  ...
  async function getClassContext(classId: string): Promise<ClassContext | undefined> {
    ...
    const cached = cache.get<ClassContext>(cacheKey);
    if (cached) return cached;
    ...
    cache.set(cacheKey, classCtx, ttl);
    return classCtx;
  }
